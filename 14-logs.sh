...
if [ $USERID -ne 0 ]
then 
    echo -e "$R ERROR:: please run this script with root acess $N" &>>LOG_FILE
...
then
 ...
 if [ $? -ne 0 ]
 then
    echo "pyhton3 is not installed...going to install it" &>>LOG_FILE
    dnf install python -y
    VALIDATE $? "python"
else 
    echo -e "python3 is alredy installed... $Y nothing to do $N" &>>LOG_FILE
fi
...
then
    ...
    echo -e "nginx is already instlled...$Y Nothing ot do $N" &>>LOG_FILE
fi